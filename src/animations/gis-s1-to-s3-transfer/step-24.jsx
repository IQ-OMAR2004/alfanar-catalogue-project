// GIS Case 1 — Step 24: "Perform the vacuum process"
// Orange vacuum machine cart connected by a black hose to the closed grey S3
// tank; the gauge needle drops as the tank evacuates, mono readout "0.8 mbar".
// Warning triangle: certified personnel only. Loop ~4.5s.
...
export default function StepAnimation({ paused = false, reduced = false }) {
  ...
  return (
    ...
    >
      <style>{`
        .c1s24-stage[data-paused] * { animation-play-state: paused !important; }

        .c1s24-needle--anim { animation: c1s24-needle 4.5s ease-in-out infinite; transform-origin: 0 0; }
        @keyframes c1s24-needle {
          0%       { transform: rotate(72deg); }
          55%,88%  { transform: rotate(-64deg); }
          100%     { transform: rotate(72deg); }
        }
        .c1s24-flow--anim { animation: c1s24-flow 1.4s linear infinite; }
        @keyframes c1s24-flow {
          0%   { stroke-dashoffset: 0; }
          100% { stroke-dashoffset: 28; }
        }
        .c1s24-run--anim { animation: c1s24-run 1.4s ease-in-out infinite; }
        @keyframes c1s24-run {
          0%,100% { opacity: 0.25; }
          50%     { opacity: 1; }
        }
        .c1s24-val--anim { animation: c1s24-val 4.5s ease-in-out infinite; }
        @keyframes c1s24-val {
          0%,50%   { opacity: 0.25; }
          60%,90%  { opacity: 1; }
          100%     { opacity: 0.25; }
        }
        .c1s24-warn--anim { animation: c1s24-warn 2.25s ease-in-out infinite; }
        @keyframes c1s24-warn {
          0%,100% { opacity: 0.65; }
          50%     { opacity: 1; }
        }
      `}</style>
      ...
      <g className={anim('c1s24-warn')} transform="translate(292 24)">
      ...
      <g transform="translate(196 96)">
        ...
        <g className={anim('c1s24-run')} style={reduced ? { opacity: 1 } : undefined}>
          <circle cx="52" cy="53" r="3.4" fill="#3FD46C" />
        </g>
        <g className={anim('c1s24-val')} style={reduced ? { opacity: 1 } : undefined}>
        ...
        <g transform="translate(50 12)">
          ...
          <g transform="translate(0 14)">
            <g className={anim('c1s24-needle')} style={reduced ? { transform: 'rotate(-64deg)' } : undefined}>
      ...
      <g className="c1s24-stage" data-paused={paused ? '' : undefined}>
        ...
        <path className={anim('c1s24-flow')} d="M 146 110 q 26 34 50 22" fill="none" stroke="#8FA6B3" strokeWidth="2.4" strokeLinecap="round" strokeDasharray="6 8" style={reduced ? { opacity: 0.5 } : undefined} />
