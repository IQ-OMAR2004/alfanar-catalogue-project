// CSD Commissioning — VLF high-voltage test with the circuit breaker ON.
// All earth wiring is removed and the earth switch is left ON. 3·Uo is applied
// for one minute to one phase with the other two earthed; the leakage current
// is read and must stay under 10 µA. Breaker closed, so the test voltage passes
// through the contact. Loop ~10s.
...
export default function StepAnimation({ paused = false, reduced = false }) {
  ...
  return (
    <Stage
      paused={paused}
      cls="cs-von"
      label="Very low frequency test set applying three times the phase voltage for one minute with the circuit breaker closed, reading a leakage current below 10 microamps"
    >
      <style>{`
        .cs-von[data-paused] * { animation-play-state: paused !important; }
        ${['r', 'y', 'b'].map((p, i) => `
        .cs-v-${p}--anim { animation: cs-v-${p} 10s linear infinite; opacity: 0; }
        @keyframes cs-v-${p} {
          0%, ${i * 33.33}%            { opacity: 0; }
          ${i * 33.33 + 1}%, ${(i + 1) * 33.33 - 1}% { opacity: 1; }
          ${(i + 1) * 33.33}%, 100%    { opacity: 0; }
        }`).join('')}
        /* VLF is a slow wave — the flow crawls compared with the IR test */
      ...
      {/* three phases — contacts CLOSED (breaker ON) */}
      <g transform="translate(48 64)">
        ...
        {[['R', 0], ['Y', 36], ['B', 72]].map(([k, cx]) => (
          <g key={k} transform={`translate(${cx} 0)`}>
            ...
            <line x1="0" y1="18" x2="0" y2="92" stroke={INK} strokeWidth="2.8" />
      ...
      <g>
        <rect x="34" y="30" width="98" height="18" rx="9" fill="#EAF4E9" stroke="#1F9D6B" strokeWidth="1.4" />
        <text x="83" y="43" textAnchor="middle" fontSize="10.5" fontWeight="700" fill="#1F9D6B"
          style={{ fontFamily: 'var(--font-mono, monospace)' }}>BREAKER ON</text>
      </g>
      ...
      {[
        ...
        <g key={p} className={reduced ? (p === 'r' ? undefined : 'cs-hide5') : anim(`cs-v-${p}`)}>
          <path
          ...
          {others.map((ox) => <circle key={`e${ox}`} cx={ox} cy="96" r="3.4" fill={INK} />)}
        </g>
      ))}
      <style>{'.cs-hide5 { display: none; }'}</style>
      ...
      <Caption x={84} y={210}>earth switch ON</Caption>
    </Stage>
  )
}
