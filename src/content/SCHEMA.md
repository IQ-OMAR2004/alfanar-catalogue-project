...
changes required. **All five languages must be present for every visible
...
export default {
  ...
  title: { en: 'Wire the LV Box', ar: '…', ur: '…', hi: '…', fr: '…' },
  summary: { en: 'One short line…', ar: '…', ur: '…', hi: '…', fr: '…' }, // card subtitle
  ...
  steps: [
    {
      ...
      title:        { en: 'Isolate power', ar: '…', ur: '…', hi: '…', fr: '…' },
      ...
      warning: { en: 'Do not proceed until the tester reads 0 V.', ar: '…', ur: '…', hi: '…', fr: '…' },
    },
  ],
}
```
...
- Fallback chain for any `{en,ar,ur,hi,fr}` field: requested → English → first
...
### Media
- A step's `media` may be a single item OR an array (rendered as a slideshow).
- `{ type: 'svg', src: 'task/step-N' }` resolves to `src/animations/<src>.jsx`.
- `{ type: 'image'|'gif'|'video', src: '/media/…' }` is a root-absolute path under
  `public/` — always pass it through `asset()` so the GitHub Pages subpath works.
